...
import Network.HTTP (Request(..), Response_String, RequestMethod(..),
                     HeaderName(..), findHeader, simpleHTTP, rspCode)
...
main = do
    ...
    badCount <- atomically $ newTVar (0 :: Int)
    badLinks <- atomically $ newTChan
    jobs <- newTChanIO
    workers <- newTVarIO k
...
--     bail = return . Left
getStatus = runErrorT . chase (5 :: Int)
  where
...
getHead :: URI -> IO (Result Response_String)
...
worker badLinks jobQueue badCount = loop
  where
    ...
    checkOne url = case parseURI url of
        Just uri -> do
            code <- getStatus uri `catch` (\(e :: SomeException) -> return . Left . show $ e)
            case code of
                Right 200 -> return ()
